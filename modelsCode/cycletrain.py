...
def sampleData(func):
    def wrapper(*args, **kwargs):
        ...
        samples = sampling(result, STS["Cols"], STS["active"])
        return samples
    return wrapper
...
class cycletrainer():
    ...
    def train(self, x0, x1):
        torch.manual_seed(torch.randint(low=1, high=999999999, size=(1,)).item())#产生全局随机种子，保证一轮训练中的随机性一致
        lossvec = []
        # 训练D0
...
#     for trainname in [cycleRGANTrainer]:
